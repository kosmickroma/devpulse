"""
Deutsche Welle Source - Implements unified search interface for DW News.

Fetches international news articles from Deutsche Welle RSS feed.
Based on POC: Deutsche Welle RSS spider (2025-11-27)
"""
...
class DeutscheWelleSource(SearchSource):
    """Deutsche Welle news search implementation using RSS feed."""

    def __init__(self):
        """Initialize Deutsche Welle RSS feed."""
        self.feed_url = "https://rss.dw.com/xml/rss-en-all"
    ...
    async def search(
        ...
        **filters
        ...
        """
        Search Deutsche Welle articles with relevance scoring.
    ...
    def _sync_search(self, query: str, limit: int) -> List[SearchResult]:
        ...
        Fetches articles from DW RSS feed with rich content extraction.
        """
        all_articles = []
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"⚠️ Deutsche Welle feed error: {response.status_code}")
                return []

            soup = BeautifulSoup(response.content, 'xml')
            items = soup.find_all('item')

            print(f"✅ Deutsche Welle: Found {len(items)} items in feed")
            ...
            for item in items:
                ...
                # Extract rich description from <content:encoded>
                ...
                if content and content.string:
                    ...
                    for tag in clean_soup(["script", "style", "iframe", "figcaption"]):
                        tag.decompose()
                    text = clean_soup.get_text(separator=' ', strip=True)
                    description = text[:1500] + ("..." if len(text) > 1500 else "")

                # Extract author - strip email addresses
                author = "Deutsche Welle"
                author_tag = item.find('author')
                if author_tag and author_tag.text:
                    # Split on space, <, or ( to remove email
                    name = re.split(r'[\s<(]', author_tag.text.strip(), 1)[0]
                    if name and name.lower() != "dw.com":
                        author = name.strip()
            ...
            print(f"⚠️ Deutsche Welle feed error: {e}")
            return []

        print(f"✅ Deutsche Welle: Total {len(all_articles)} articles from feed")
        ...
        print(f"✅ Deutsche Welle: Returning {len(results)} relevant articles")
        return results
